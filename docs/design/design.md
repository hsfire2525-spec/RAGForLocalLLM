...
### `rag sweep` — 手回しを仕組みにした

予算の調整（§9 Phase 3-2）はシェルのループで設定を生成する手回しだった。
同じことを繰り返すなら仕組みにしたほうがよい。手回しの問題は3つある。

1. **設定がどこにも残らない** — 何を比較したのか後から追えない
2. **失敗に気付きにくい** — 途中で落ちたランが黙って欠ける
3. **再インデックスの要否が読めない** — `index` 側を触る軸は再構築が要る

`configs/sweeps/*.yaml` に軸を書き、`rag sweep` で回す。既定はアブレーション
（1軸ずつ変える）。実装で対処した点:

- **全変種を実行前に検証する。** 30分のスイープの28分目で設定エラーが出るのが最悪
- **基準と同一になる変種は畳む。** 見通しのため各軸に基準値を並べるのは自然だが、
  素朴に展開すると基準と同じランが軸の数だけ増える（実測で10件中3件が重複）
- **`type` を変えたら前の実装の引数を捨てる。** `{type: hybrid, rrf_k: 60,
  retrievers: [...]}` の type を dense にすると、残った引数をレジストリが拒否する。
  `extends` の差分継承と同じ扱いにした（core/config.py と同じ理由）
- **1件の失敗でスイープ全体を捨てない。** 残りを走らせてから失敗をまとめて報告する

#### 検索まわりのアブレーション（`configs/sweeps/retrieval.yaml`、gold `qa_v2` 60問）

| 構成 | 正答率 | 基準との差 [95%CI] | hit@5 | context_hit |
| --- | --- | --- | --- | --- |
| **hybrid_budget（基準）** | **0.983** | — | 0.960 | 0.960 |
| retriever=dense | 0.917 | **-0.067 [-0.133, -0.017]（悪化）** | 0.880 | 0.880 |
| retriever=sparse | 0.883 | **-0.100 [-0.183, -0.033]（悪化）** | 0.900 | 0.900 |
| budget=1536 | 0.950 | -0.033 [-0.083, 0.000] | 0.960 | 0.920 |
| budget=4096 | 0.983 | 0.000 | 0.960 | 0.960 |
| rrf_k=10 | 0.967 | -0.017 [-0.050, 0.000] | 0.940 | 0.940 |
| rrf_k=120 | 0.967 | -0.017 [-0.050, 0.000] | 0.940 | 0.940 |

読み取れること:

- **dense 単体・BM25 単体はどちらも有意に悪い。** 併用が効いており、
  片方だけでは足りない。BM25 単体（0.883）が dense 単体（0.917）より悪いのは、
  語彙が重ならない質問を落とすため
- **`rrf_k` は 10〜120 の範囲でほとんど効かない。** 既定の 60 で十分であり、
  ここを調整しても得るものがない。**触らなくてよい軸が分かったこと自体が成果**
- **budget=4096 は 2560 と完全に同じ。** 予算をさらに増やす価値はない

#### UI の同時アクセス（実測）

サーバはパイプラインの実行を直列化している（埋め込みモデルと LM Studio の
同時要求を避けるため）。その待ち時間を測った。

| 同時人数 | 全体 | 1人あたりの待ち（中央値 / 最長） |
| --- | --- | --- |
| 1 | 2.9s | 2.9s / 2.9s |
| 2 | 5.4s | 4.2s / 5.4s |
| 4 | 5.9s | 2.3s / 5.9s |

**4人同時でも最長6秒**で、想定される社内利用（数人が散発的に使う）には
十分。4人が2人の倍にならないのは、LM Studio 側が短い回答を速く返すため。
**並列化は現時点では不要**であり、利用者が増えて待ち時間が問題になってから
実測に基づいて手を入れればよい。
